...
class QrCameraC2 implements QrCamera {
    ...
    QrCameraC2(int width, int height, Context context, SurfaceTexture texture, QrDetector2 detector, boolean front) {
    ...
    public void start() throws QrReader.Exception {
        ...
        try {
            ...
            for (String id : cameraIdList) {
                ...
                if (integer != null && integer == LENS_FACING_BACK && !front) {
                    cameraId = id;
                    break;
                }
                else if (integer != null && integer == LENS_FACING_FRONT && front) {
                    cameraId = id;
                    break;
                }
            }
        } catch (CameraAccessException e) {
            Log.w(TAG, "Error getting camera.", e);
            throw new RuntimeException(e);
        }

        if (cameraId == null && !front) {
