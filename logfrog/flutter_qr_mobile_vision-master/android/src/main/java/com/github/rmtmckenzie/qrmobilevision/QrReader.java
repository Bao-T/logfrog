...
class QrReader {
    ...
    QrReader(int width, int height, Activity context, int barcodeFormats,
             ...
             final SurfaceTexture texture, boolean front) {
        this.context = context;
        this.startedCallback = startedCallback;

        if (android.os.Build.VERSION.SDK_INT >= 21) {
            Log.i(TAG, "Using new camera API.");
            qrCamera = new QrCameraC2(width, height, context, texture, new QrDetector2(communicator, context, barcodeFormats), front);
        } else {
            Log.i(TAG, "Using old camera API.");
            qrCamera = new QrCameraC1(width, height, texture, new QrDetector(communicator, context, barcodeFormats), front);
        }
    }
