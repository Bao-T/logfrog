...
class QrCameraC1 implements QrCamera {
    ...
    QrCameraC1(int width, int height, SurfaceTexture texture, QrDetector detector, boolean front) {
    ...
    public void start() throws QrReader.Exception {
        ...
        for (int i = 0; i < numberOfCameras; i++) {
            ...
            else if (info.facing == Camera.CameraInfo.CAMERA_FACING_BACK && !front){
                camera = Camera.open(i);
                break;
            }
        }

        if (camera == null && !front) {
